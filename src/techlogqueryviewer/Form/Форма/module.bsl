...
Функция ПолучитьФайлыДляОбработки(ДатаНачала)
	...
	Файлы = НайтиФайлы(ПолучитьПутьКЛогам(),"????????*.log", Истина);
...
Процедура ПрочитатьЛоги()
	...
	Для Каждого Строка из НовыеСобытия Цикл
		Если НетОтобраПоТексту ИЛИ Найти(НРЕГ(Строка.Текст), ОтборПоТекстуНРЕГ) > 0 Тогда 
			мКоличествоЗапросовНаВывод = мКоличествоЗапросовНаВывод + 1;
			Результат.ДобавитьСтроку(ПолучитьТекстДляВывода(Строка));
		КонецЕсли;
...
Процедура СтартНажатие(Элемент)
	...
	Если мИдетОбработка Тогда
		...
		Если мБылСозданФайлНастроек Тогда
			Попытка
				УдалитьФайлы(ПолучитьПутьКФайлуНастроекЖурнала());
			Исключение
			КонецПопытки;
		КонецЕсли;
	Иначе
		// Создадим файл настроек технологического журнала, если существует, то сделаем бэкап
		ПутьКФайлуНастроекЖурнала = ПолучитьПутьКФайлуНастроекЖурнала(); 
		Файл = Новый Файл(ПутьКФайлуНастроекЖурнала);	
		Если Файл.Существует() Тогда
			мИмяФайлаБэкапаНастроекЖурнала = ПолучитьУникальноеИмяФайла(КаталогПрограммы() + "\conf\backup_logcfg.xml");
			ПереместитьФайл(ПутьКФайлуНастроекЖурнала,  мИмяФайлаБэкапаНастроекЖурнала); 	
		КонецЕсли;
		
		мБылСозданФайлНастроек = СоздатьФайлНастроек(ПутьКФайлуНастроекЖурнала, ПолучатьПланыЗапросов);
...
Процедура ПриЗакрытии()
	...
	Если мИмяФайлаБэкапаНастроекЖурнала <> Неопределено Тогда
		...
		Если ФайлБэкапа.Существует() Тогда
			ПереместитьФайл(мИмяФайлаБэкапаНастроекЖурнала, ПолучитьПутьКФайлуНастроекЖурнала()); 
		КонецЕсли;
	КонецЕсли;
