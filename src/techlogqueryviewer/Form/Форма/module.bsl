	...
	Иначе
		...
		Если мИдетОбработка Тогда
			...
			Для Счетчик = 1 по мКоличествоТочекАнимиации Цикл
				Анимация = Анимация + ".";
			КонецЦикла;
			ЭлементыФормы.НадписьКаталогЛоговАнимация.Заголовок = NStr("en='Log folder is not created yet.';ru='Каталог журнала еще не создан.'") + Анимация;
...
Процедура ЗаполнитьСоответствияИмен()
	...
	Для каждого Строка из СтруктураХраненияБазыДанных Цикл
		...
		If Metadata.ScriptVariant = Metadata.ObjectProperties.ScriptVariant.English Then
			ИмяТаблицы = Строка.TableName;
			Если ЗначениеЗаполнено(ИмяТаблицы) Тогда
				НоваяСтрока = мСоответствияИмен.Добавить();
				ИмяТаблицыХранения = Строка.StorageTableName;
		...
		Для каждого Поле из Поля Цикл
			If Metadata.ScriptVariant = Metadata.ObjectProperties.ScriptVariant.English Then			
				ИмяПоля = Поле.FieldName;
				Если ЗначениеЗаполнено(ИмяПоля) Тогда 
					НоваяСтрока = мСоответствияИмен.Добавить();
					НоваяСтрока.СУБД = Поле.StorageFieldName;					
...
Процедура УстановитьОтбор()
	...
	ЭлементыФОрмы.НадписьЗапросов.Заголовок = NStr("en='Queries: ';ru='Запросов: '") + Строка(мУжеПрочитанныеСобытия.Количество()) + ?(НетОтобраПоТексту, "", NStr("en=' (Filtered: ';ru=' (Отобрано: '") + Строка(мКоличествоЗапросовНаВывод) + ")");
...
Процедура СтартНажатие(Элемент)
	...
	Если мИдетОбработка Тогда
		...
		Заголовок = NStr("en='Techlogqueryviewer (stopped)';ru='Вывод запросов из технологического журнала (остановлено)'");
		
		мИдетОбработка = Ложь;
		ЭлементыФормы.Старт.Заголовок = NStr("en='Start';ru='Старт'");
	...
	Иначе
		...
		Если Не мБылСозданФайлНастроек Тогда
			...
			Если Не Файл.Существует() Тогда
				...
				Если мПутьКФайлуНастроекЖурнала <> Неопределено Тогда
					...
					мБылСозданФайлНастроек = СоздатьФайлНастроек(мПутьКФайлуНастроекЖурнала, ПолучатьПланыЗапросов);
					Если Не мБылСозданФайлНастроек Тогда
						Заголовок = NStr("en='Techlogqueryviewer (Failure of creating log settings file)';ru='Вывод запросов из технологического журнала (Не удалось создать файл настроек)'");
						Предупреждение(NStr("en='Failure of creating log settings file';ru='Не удалось создать файл настроек'"));
						мИдетОбработка = Ложь;
						Возврат;
					КонецЕсли;
				КонецЕсли;
		...
		Заголовок = NStr("en='Techlogqueryviewer (processing)';ru='Вывод запросов из технологического журнала (идет обработка)'");
		...
		ЭлементыФормы.Старт.Заголовок = NStr("en='Stop';ru='Стоп'");				 
	КонецЕсли;
...
Процедура ПриЗакрытии()
	...
	СохранитьЗначение("Обработка.techlogqueryviewer", Настройки);
...
Процедура ОчиститьНажатие(Элемент)
	...
	мДатаПриСтарте = ТекущаяДата();
	мКоличествоЗапросовНаВывод = 0;
	ЭлементыФОрмы.НадписьЗапросов.Заголовок = NStr("en='Queries: 0';ru='Запросов: 0'");
...
Процедура ПриОткрытии()
	...
	Настройки = ВосстановитьЗначение("Обработка.Techlogqueryviewer");
...
Процедура ПредставлениеПутиККаталогуЛоговНажатие(Элемент)
	...
	Иначе		
		Предупреждение(NStr("en='No such folder.';ru='Каталог не существует.'"));
	КонецЕсли;
